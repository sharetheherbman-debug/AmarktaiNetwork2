...
type Status = 'live' | 'invite_only' | 'in_development' | 'coming_soon' | 'concept'

const statusConfig: Record<Status, { label: string; className: string }> = {
  live: { label: 'Live', className: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
  invite_only: { label: 'Invite Only', className: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
...
export default function StatusBadge({ status, className }: StatusBadgeProps) {
  const config = statusConfig[status as Status] || { label: status, className: 'bg-slate-500/15 text-slate-400 border-slate-500/30' }
